...
offset=5
enddate_value=$(date -u -d "now + $offset minutes" '+%y%m%d%H%M%SZ')
# Generate normal operation short_lived_domain_proxy certificate/key.
echo "\n\nGenerate 'short_lived_domain_proxy' certificate/key"
openssl req -new -newkey rsa:2048 -nodes \
    -reqexts oper_req -config ../../../cert/openssl.cnf \
    -out short_lived_domain_proxy.csr -keyout short_lived_domain_proxy.key \
    -subj "/C=US/ST=District of Columbia/L=Washington/O=Wireless Innovation Forum/OU=www.wirelessinnovation.org/CN=SAS Domain Proxy Example"
openssl ca -cert proxy_ca.cert -keyfile private/proxy_ca.key -in short_lived_domain_proxy.csr \
