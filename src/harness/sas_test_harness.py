...
class SasTestHarnessServer(threading.Thread):
  ...
  def __init__(self, name, host_name, port, sas_version, dump_path, cert_file=None, key_file=None):
    ...
    Args:
        ...
        base_url: base url of SAS Test Harness that the SAS UUT should connect to for FAD records.
  ...
  def generateCbsdRecords(cbsd_records, grant_records_list):
    ...
    for index, (cbsd_record, grant_records) in enumerate(zip(cbsd_records, grant_records_list)):
      ...
      # Check if groupingParam optional parameter present in CBSDRecord
      # then add to registration object
      ...
      # Read all grant records if mulitiple grant records associated with single cbsd
  ...
  def writeFadRecords(base_url, dumpfile_path, dump_records_list):
    """ This method used to write dump records in specified path these files will be served
        by SAS Test Harness
    Args:
