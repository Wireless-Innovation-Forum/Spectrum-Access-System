...
class GrantTestcase(sas_testcase.SasTestCase):
  ...
  def generate_GRA_5_default_config(self, filename):
    """ Generates the WinnForum configuration for GRA 5."""

    # Load device_a and device_c for the registration in SAS Test Harness.
    device_a = json.load(
        open(os.path.join('testcases', 'testdata', 'device_a.json')))
    device_c = json.load(
        open(os.path.join('testcases', 'testdata', 'device_c.json')))
    ...
    # Load grant request for device_a and device_c in SAS UUT.
  ...
  def test_WINNF_FT_S_GRA_5(self, config_filename):
    ...
    # Create an activity dump of SAS Test Harness that includes record for CBSD C1 and Grant G1
    ...
    # Do the registration again for device_a in SAS UUT and check the response.
    cbsd_ids_a = self.assertRegistered([device_a])

    # Inject FccId and UserId of device_c in SAS UUT.
    self._sas_admin.InjectFccId({'fccId': device_c['fccId']})
    self._sas_admin.InjectUserId({'userId': device_c['userId']})

    # Do the registration for device_c in SAS UUT and Check response of device_c in SAS UUT.
    cbsd_ids_c = self.assertRegistered([device_c])

    # Send a Grant request for device_a which is already granted from another SAS.
    grant_a_sas_uut['cbsdId'] = cbsd_ids_a[0]
    request = {'grantRequest': [grant_a_sas_uut]}
    response = self._sas.Grant(request)['grantResponse'][0]

    # Check grant response of device_a in SAS UUT.
    self.assertEqual(response['response']['responseCode'], 401)
    del request, response

    # Send grant request for device_c in SAS UUT.
    grant_c['cbsdId'] = cbsd_ids_c[0]
  ...
  def generate_GRA_6_default_config(self, filename):
    """Generates the WinnForum configuration for GRA 6."""
  ...
  def generate_GRA_6_activity_dump(self, sas_th_url, device_a, grant_a_th):
    ...
    end_time = time.strftime(end_time.replace(microsecond=0).isoformat()) + 'Z'
    start_time = time.strftime(start_time.replace(microsecond=0).isoformat()) + 'Z'
    grant_expire_time = datetime.now().today() + timedelta(hours=24)
    grant_expire_time = str(grant_expire_time.replace(microsecond=0).isoformat()) + 'Z'
    dump_file['startTime'] = start_time
    dump_file['endTime'] = end_time
    dump_file['recordData'] = []
    internal_data = {
      ...
      'grants': [{
          ...
          'channelType': 'GAA',
          'grantExpireTime': grant_expire_time,
          'terminated': False}]}
    dump_file['recordData'].append(internal_data)
    device_a_id = dump_file['recordData'][0]['id']
  ...
  def test_WINNF_FT_S_GRA_6(self, config_filename):
    ...
    # Do the registration for device_a and check response should be 0
    cbsd_ids_a = self.assertRegistered([device_a])
