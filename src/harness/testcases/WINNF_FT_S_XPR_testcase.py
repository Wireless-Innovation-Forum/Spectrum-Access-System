...
class AggregateInterfaceProtectionTestcase(sas_testcase.SasTestCase):
  ...
  def generate_XPR_1_default_config(self, filename):
    ...
    # Moving device_1 to a location within 40 meters of FSS
    device_1['installationParam']['latitude'] = 39.22904
    device_1['installationParam']['longitude'] = -100.09994

    device_2 = json.load(
      open(os.path.join('testcases', 'testdata', 'device_b.json')))
    # Moving device_2 to a location outside 40 meters of FSS
    device_2['installationParam']['latitude'] = 39.22906
    device_2['installationParam']['longitude'] = -100.09869

    device_3 = json.load(
      open(os.path.join('testcases', 'testdata', 'device_c.json')))
    # Moving device_3 to a location outside 40 meters of FSS
    device_3['installationParam']['latitude'] = 39.22827
    device_3['installationParam']['longitude'] = -100.1

    device_4 = json.load(
      open(os.path.join('testcases', 'testdata', 'device_d.json')))
    # Moving device_4 to a location outside 40 meters of FSS
    device_4['installationParam']['latitude'] = 39.22867
    device_4['installationParam']['longitude'] = -100.09949
  ...
  def generate_XPR_2_default_config(self, filename):
    """ Generates the WinnForum configuration for XPR.2. """
    # Load devices info
    device_1 = json.load(
      open(os.path.join('testcases', 'testdata', 'device_a.json')))
  ...
  @configurable_testcase(generate_XPR_2_default_config)
  def test_WINNF_FT_S_XPR_2(self, config_filename):
      """Multiple CBSDs from Multiple SASs Inside and 
         Outside the Neighborhood of the Protected Entity
      """
      config = loadConfig(config_filename)
      # TODO
      # test_type= enum (MCP, XPR)
