...
class GrantTestcase(unittest.TestCase):
  ...
  def test_WINFF_FT_S_GRA_2(self):
	...
	responseZone = self._sas_admin.InjectEscZone([esc_zone_not_contain_device_a])
    self._sas_admin.TriggerEscZone({'zone_id': responseZone['zone_id'],
    ...
    # Check grant response
    self.assertEqual(response['cbsdId'], cbsd_id)
    self.assertTrue(response['grantId'] in response )
    self.assertEqual(response['channelType'], 'GAA')
    self.assertEqual(response['response']['responseCode'], 0)
...
@winnforum_testcase
  def test_WINFF_FT_S_GRA_3(self):
    ...
    del request, response
    # Create and trigger the ESC Zone
    responseZone = self._sas_admin.InjectEscZone({'zoneDate' :[esc_zone_contains_device_a])
    self._sas_admin.TriggerEscZone({'zone_id': responseZone['zone_id'],
                                    'operationFrequencyRange': {
                                     'lowFrequency': 3620000000.0,
                                     'highFrequency': 3630000000.0 }})
