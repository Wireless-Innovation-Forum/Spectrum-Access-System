...
class GrantTestcase(sas_testcase.SasTestCase):
  ...
  def test_WINNF_FT_S_GRA_13(self):
    """Send grant request for device with cbsdId different from its assigned cbsdId,
        SAS should reject grant.
    ...
    # Create grant request for device_a using cbsd_id_c
    grant_0 = json.load(
        open(os.path.join('testcases', 'testdata', 'grant_0.json')))
    grant_0['cbsdId'] = cbsd_id_c
    request = {'grantRequest': [grant_0]}
    # Check grant Response
    response = self._sas.Grant(request,device_a_cert, device_a_key)['grantResponse'][0]
    self.assertFalse('cbsdId' in response)
    self.assertFalse('grantId' in response)
  ...
  @winnforum_testcase
  def test_WINNF_FT_S_GRA_18(self):
