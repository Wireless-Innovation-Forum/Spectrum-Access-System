...
class TestZonePurge(unittest.TestCase):
  ...
  def test_zone_PurgeModel_default(self):
    ...
    # TEST DATA
    cbsd_0 = json_load(os.path.join(TEST_DIR, 'testset1', 'cbsd_0.json'))
    cbsd_1 = json_load(os.path.join(TEST_DIR, 'testset1', 'cbsd_1.json'))
    cbsd_2 = json_load(os.path.join(TEST_DIR, 'testset1', 'cbsd_2.json'))
    cbsd_3 = json_load(os.path.join(TEST_DIR, 'testset1', 'cbsd_3.json'))
    cbsd_4 = json_load(os.path.join(TEST_DIR, 'testset1', 'cbsd_4.json'))
    cbsd_5 = json_load(os.path.join(TEST_DIR, 'testset1', 'cbsd_5.json'))
    # Load PPA record
    gwpz_record = json_load(
        os.path.join(TEST_DIR, 'testset1', 'gwpz_0.json'))
    ppa_record = json_load(
        os.path.join(TEST_DIR, 'testset1', 'ppa_0.json'))
    pal_record_0 = json_load(
        os.path.join(TEST_DIR, 'testset1', 'pal_0.json'))
    pal_record_1 = json_load(
        os.path.join(TEST_DIR, 'testset1', 'pal_1.json'))
    pal_record_2 = json_load(
        os.path.join(TEST_DIR, 'testset1', 'pal_2.json'))

    pal_record_list = [pal_record_0, pal_record_1, pal_record_2]
    fss_record = json_load(
        os.path.join(TEST_DIR, 'testset1', 'fss_0.json'))
    gwbl_record = json_load(
        os.path.join(TEST_DIR, 'testset1', 'gwbl_0.json'))

    pal_low_frequency = pal_record_0['channelAssignment']['primaryAssignment'][
        'lowFrequency']
    pal_high_frequency = pal_record_0['channelAssignment']['primaryAssignment'][
        'highFrequency']
    ppa_record, pal_records = util2.makePpaAndPalRecordsConsistent(
        ppa_record, pal_record_list, pal_low_frequency, pal_high_frequency,
        'test_user_1')
    ppa_record['ppaInfo']['cbsdReferenceId'] = ['cbsd_5', 'cbsd_3']
    fad_object_1 = full_activity_dump.FullActivityDump(
        {'cbsd': [cbsd_0, cbsd_1]})
    fad_object_2 = full_activity_dump.FullActivityDump(
        {'cbsd': [cbsd_2, cbsd_3]})
    fad_object_3 = full_activity_dump.FullActivityDump(
        {'cbsd': [cbsd_4, cbsd_5]})
    ...
    print(
        '================CBSD Grants received as output======================')
    final_grants = 0
    for records in sas_uut_fad.getCbsdRecords():
      for grants in records['grants']:
        print(' ', json.dumps(grants['id']))
