...
def getFssInfo(fss_record):
  ...
  fss_pointing_elevation = - fss_record['record']['deploymentParam'][0]['installationParam']\
                              ['antennaDowntilt']
...
def generatePurgeListForFssPoint(cbsds, fss_entity):
  ...
  Args:
    ...
    fss_entity: The FssProtectionPoint tuple of the FSS for which the purge is 
                to be performed.
  Returns:
    List of multiple key-value pairs with keys "grantData", "cbsd" and values 
      as grants to be
  ...
  oobe_threshold_value = -129 + 10 * \
                           (np.log10(REF_BW/interference.ONE_MHZ) - FSS_OOBE_MARGIN)
  ...
  while True:
    ...
    # the OOBE threshold.
    oobe_interferences = [interference.dbToLinear(grant['oobe_interference'])\
    ...
    index = np.searchsorted(cumulated_interference,\
              interference.dbToLinear(oobe_threshold_value))
      ...
      for purged_grant in grants_to_purge:
        ...
        for cbsd_grant in purged_grant['cbsd']['grants']:
          ...
          if (purged_grant['mcbsd']) == getMcbsdValue(cbsd_grant):
            final_purge_list.append(grants_cbsds_namedtuple\
                               (cbsd_grant, purged_grant['cbsd']))
          else:
            # Find the grants not matching the mcbsd values and not present in 
            # the purge list and add it to the remaining grants list
        ...
        # Identify the grant with the highest highFrequncy value among the remaining 
        ...
        if remaining_grants:
          max_grant = max(remaining_grants,\
                        key=lambda x: x['operationParam']['operationFrequencyRange']\
                        ['highFrequency'])
          grants_cbsds_info_for_oobe_calculation.\
                  append({'mcbsd': getMcbsdValue(max_grant),'cbsd': purged_grant['cbsd']})
...
def calculateOobeInterference(grants_cbsds_oobe_info, fss_entity):
  ...
  Args:
    grants_cbsds_oobe_info : List of dictionaries contianing the highest grant of their
                             CBSD and a reference to the CBSD.
    fss_entity: The FssProtectionPoint tuple of the FSS for which the interference 
                is calculated.
  """
  ...
  for grant_cbsds_oobe_info in grants_cbsds_oobe_info:
    if grant_cbsds_oobe_info['cbsd']['grants']:
      lcbsd, incidence_angle = getMeanPathLossFromCbsdToFss\
                                 (grant_cbsds_oobe_info['cbsd'], fss_entity)
...
def fssPurgeReferenceModel(sas_uut_fad, sas_test_harness_fads, fss_records):
  ...
  for fss_record in fss_records:
    ...
    if fss_record['ttc']:
      neighboring_cbsds_with_grants = pre_iap_util.\
                                        getFssNeighboringCbsdsWithGrants(cbsds, fss_record, 40)
