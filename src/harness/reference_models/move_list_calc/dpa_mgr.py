...
class Dpa(object):
  ...
  def CheckInterference(self, channel, sas_uut_active_grants, margin_db, num_iter=None):
    ...
    Args:
      channel: A channel as tuple (low_freq_mhz, high_freq_mhz).
      sas_uut_active_grants: A list of authorized |data.CbsdGrantInfo| grants
    ...
    pool = mpool.Pool()
    result = pool.map(_CheckPointInterf, self.protected_points)
