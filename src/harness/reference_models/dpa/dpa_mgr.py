...
class Dpa(object):
  ...
  def CheckInterference(self, sas_uut_active_grants, margin_db,
    ...
    if self.apply_clutter_network_loss_and_50_percent:
...
def GetDpaProtectedChannels(freq_ranges_mhz, is_esc_dpa=True):
  ...
  Args:
    freq_ranges_mhz: The protection frequencies (MHz) as a list of tuple
      (freq_min_mhz, freq_max_mhz) of the DPA protected frequency ranges.
    is_esc_dpa: True if Coastal ESC DPA.
...
def BuildDpa(dpa_name, protection_points_method=None, dpa_filename=None,
  ...
  try:
    ...
    try:
      dpa_zone = zones.GetAnyDpaZones(kml_path=dpa_filename)[dpa_name]
      is_esc = False
    except KeyError:
