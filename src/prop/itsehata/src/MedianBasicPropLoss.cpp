...
void MedianBasicPropLoss(float f__mhz, float h_b__meter, float h_m__meter, float d__km, int enviro_code, float* plb_med__db, InterValues *interValues)
{
    ...
    double n_h = 2.0*(rho + log10(h_b__meter)*(sigma + tau*log10(h_b__meter)) - 1.0); // n_h = 2 * (rho + sigma * log10(h_b__meter) + tau * pow(log10(h_m__meter), 2) - 1);
	double n_l = 0.1 * (44.9 - 6.55*log10(h_b__meter)) - 2.0;  // NOTE! 44.9 OR 24.9????  And why the -2??
	...
	double sr_d = sqrt(1.0e+6 * pow(d__km, 2) + pow(h_b__meter - h_m__meter, 2)); // BK: distance from base station to mobile, along the ray(compute triangle hypoth)
